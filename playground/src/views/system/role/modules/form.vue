...
const [Drawer, drawerApi] = useVbenDrawer<Partial<Row>>({
  async onOpenChange(open) {
    ...
    try {
      ...
      current.value = data?.id ? await getDetail('roles', data.id) : undefined;
      await formApi.reset();
      formApi.setState({ schema: schemaFor('roles', current.value) });
      await nextTick();
  ...
  async onConfirm() {
    ...
    drawerApi.lock();
    try {
      await saveRecord('roles', await formApi.getValues(), current.value);
      emit('success');
      drawerApi.close();
    } finally {
      drawerApi.unlock();
...
const title = computed(() => `${current.value ? '编辑' : '新增'}角色`);
</script>
<template>
